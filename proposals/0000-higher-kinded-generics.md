...
* Constructors with rank-n types, as in `data Foo = MkFoo (forall a. a
  ...
  Caveat: due to GHC's inability to bind type variables in
...
* Any data type with a kind that uses nested `forall`s (e.g., `data Foo :: forall a. a -> forall b. b -> Type`) or is otherwise
